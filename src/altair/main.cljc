(ns altair.main
  "Kotodama WASM entrypoint for the Altair clean-room actor (L4) — Clojure port.
  ...
  facts (`altair.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "altair")
(def tier "L4")
...
(def entity-specs
  [{:entity "Project"   :plural "projects"   :id-prefix "altair_pro"
    :fields [:jurisdiction :status :name]    :required [:name :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Permit"      :plural "permits"      :id-prefix "altair_per"
    :fields [:projectId :type :number :status] :required [:type :number]
    :coerce {}                       :refs {:projectId "Project"}}
   {:entity "Drawing"     :plural "drawings"     :id-prefix "altair_dra"
    :fields [:projectId :name :revision :contentRef] :required [:name :revision]
    :coerce {}                       :refs {:projectId "Project"}}
   {:entity "Inspection"  :plural "inspections"  :id-prefix "altair_ins"
    :fields [:permitId :type :result :inspectedAt] :required [:type :result]
    :coerce {}                       :refs {:permitId "Permit"}}
   {:entity "Asset"       :plural "assets"       :id-prefix "altair_ass"
    :fields [:name :type :location :condition] :required [:name :type]
    :coerce {}                       :refs {}}
   {:entity "ServiceRequest" :plural "servicerequests" :id-prefix "altair_ser"
...
(defn emit-facts
  "EAVT facts for one record: {\"altair.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "altair-compat" :tier tier :entities entities} 200])
...
(defn start! [] :altair-compat/ready)
